...
Algumas coisas saltam aos olhos nesse código:
  ...
  - Da linha 2 até a linha 5 está uma parte do texto (delimitda pelo elemento `head`). 
    ...
    mas é de extrema importancia para configurações;
...
Por curiosidade, um das características do elemento "a" (*anchor*, que significa
...
o html.
Ficaremos com aqueles que usaremos nesse minicurso [^mozilla]
...
Com essas informações, já estamos minimamente familizarizados com o html 
...
Caso queira fazer um teste, copie e cole o código abaixo num bloco de notas e salve em html por nome, por exemlo, `index.html`.
...
da figura seja `png`).
Depois abra-o com seu navegador de internet padrão.
...
Bom, mas convenhamos ... fica feio, não?
...
Ao ativar a ferramenta, retângulos coloridos serão exibidos para que selecione
o objeto desejado.
Mas, geralmente, quando fazemos isso no primeiro "clique", outras partes são 
selecionadas que não desejamos.
Então, clicamos novamente sobre as partes que não desejamos!

> Veja: podem ocorrer dusa etapas! 
> Na primeira, selecionamos o que desejamos.
> Se algum elemento não desejamo estiver também selecionado, clicamos nele em seguida.
...
Nosso objetivo é selecionar a região onde se encontram os livros.
  ...
  ficará um retângulo de borda "vermelha" englobando a imagem, o título, os valores
  e o botão do carrinho de compras).
  3. Clicamos nessa região. 
  Em "verde" ficará a primeira região que clicamos e em amarelo TUDO o que 
  desejamos selecionar.
  Num retêngulo inferior de exibição do SelectorGadget aparecerá o elemento CSS:
  "**.col-lg-3**".
  4. Quer dizer ... nem tudo está correto! 
  Englobamos coisas a mais!
  Basta rolar o cursor do *mouse* até o final da página e veremos que uma pequena
  região do "cartão de crédito" foi selecionada indevidamente!
