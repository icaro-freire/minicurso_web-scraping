...
Por isso, dependendo da época que você esteja lendo esse texto, as coisas podem
estar diferentes.
...
área do site ou do próprio site!
...
Uma das fromas de sabermos se um site permite a raspagem é verificando os **robots.txt**.
...
Agora estamos prontos para a manipulação desses dados!
...
Abaixo, o código mostra que o elemento CCS "sp-component li li a"
...
Então, basta usar a função `rvest::html_attr("href")`:
...
Antes de prosseguirmos note, que há um certo "padrão" que aparece nos links que 
desejamos.
Por exemplo, antes do nome dos arquivos que desejamos extrair, podemos observar
a repetição dos caracteres "**CIR.**".
Isso seria importante se a nossa seleção abrangesse mais arquivos do que desejamos.
Caso isso ocorresse, poderíamos usar uma função do pacote `stringr` para 
...
Porém, vamos organizar as coisas ...
...
diretório mais geral, denominado **download_arquivos/**, onde salvaremos os 
arquivos dos sites específicos estudados.

Por exemplo, para os arquivos desse site da UESC, criaremos um subdiretório por 
nome "arquivos_uesc/".
Vamos atribuir a variável **arquivos_uesc** todo esses procedimentos (note que
...
No site "https://www.obm.org.br/revista-eureka/"
