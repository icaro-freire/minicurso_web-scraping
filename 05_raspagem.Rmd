...
ser melhor...
Por exemplo, seria possível criarmos um programa que exibisse uma perguta sobre 
a porcentagem do desconto; e, depois de digitado o valor, ele retornasse a tabela 
de decisão?
Sim! Isso é possível!
...
<p align='center'>
[`bow(url, user_agent = ...)`](https://dmi3kno.github.io/polite/reference/bow.html).
</p>
...
São 25 páginas, mas é importante fazermos a raspagem de uma delas inicialmente 
(no caso, a primrira) para visualizarmos as estruturas básicas de repetição; e, 
assim, formarmos a ideia de generalização às outras páginas.
...
Isso é importante, pois é possível fazer cálculos com datas, mas não com caracteres.

Feito isso, como exibir esses dados de forma conveniente?
Bom, uma tabela já seria suficiente para nossos fins:
...
Isso é, basicamente, o que desejamos fazer em cada uma das 25 páginas.
Então, é natural perguntar-nos: o que muda de uma página para outra, visto que 
essas estruturas do CSS são mantidas? 
Como iterar nas outras páginas?

### 5.4.2 Raspagem nas 25 páginas

Para responder a pergunta antrior, você deve encontrar algum padrão na `url` de cada
página.
...
O que mudou?
Ora, apenas o número da página (de $0$ até $4$).
Se denominarmos esse número da página por `nPg`, podemos separar as `urls` em 
três partes:

1. "https://www.mises.org.br/Articles_Thumbs.aspx?page="
2. nPg
3. "&type=0&text="

> Como são 25 páginas, `nPg` terá uma variação de $0$ até $24$

Essas informações são muito importantes; pois, vimos que, para a iteração ocorrer
com as funções do pacote `purrr`, devemos criar uma função específica com as 
propriedades que desejamos na raspagem.
É o que faremos, passo a passo, a seguir.

Vamos denominar a função que faz a raspagem dos dados de uma página genérica do
site Mises Brasil, por **`raspa_mises`**, cuja variável será `nPg` (número da 
página): `raspa_mises(nPg)`.
Assim, se desejo extrair a _data_, o _autor_ e o _título_ de todos os artigos que 
estão na página 17, por exemplo, faço: `raspa_mises(17)`.

Mas, como seria essa função?

1. Bom, a primeira coisa a se fazer é preparar a `url` do site dessa página.
Como vimos, podemos separar em 3 partes e a função `str_c()`, do pacote `stringr`  
pode concatenar essas partes.
2. Em seguida, fazermos as requisições necessárias e extraírmos o html da página
para nosso computador. Como são muitas páginas, usaremos o pacote `polite`, com
suas funções **`bow()`** e **`scrape()`**.
3. Por fim, seguimos os passos da extração que foi realizada na primeira página.
...
Qual seriam as 7 primeiras linhas da saída dessa função, quando aplicada à 
primeira página?
Vimos que a primeira página começa do "0"; logo, devemos calcular `raspa_mises(7)`,
além se pinçar as sete primeiras linhas, como no código abaixo:
...
Note que atribuí à variável `tabela_mises_total` à saída do resultado, ou seja,
uma tabela, com três colunas (data, autor e título) das 25 páginas do site.
Se você quiser exibir essa tabela no console do programa, basta digitar:

```{r, eval=FALSE}
tabela_mises_total
```

Porém, é interessante salvar essa tabela em um arquivo `.csv`, para que você crie
uma base de dados que poderá ser usado em uma análise separada.
...
O _script_ completo pode ser visto abaixo.
