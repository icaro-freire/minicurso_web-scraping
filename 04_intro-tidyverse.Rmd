...
description: |
  Explanação sucinta apenas das funções que usaremos do tidyverse.
output: 
...
- **readr**; só usaremos a função `parse_double()` que, como o nome sugere, 
  passa um conjunto de caracteres para a categoria numérica. Uma função do R Base
  que poderia fazer o mesmo papel seria `as.numeric()`.
...
- **stringr**; para manipulação de cadeias de caracteres, como frases, texto, 
  símbolos, etc. É bem útil para "limpeza" dos dados coletados numa raspagem da
  internet (as coisas, geralmente, não vem "arrumadinhas").
- **purrr**; para interações. Substitui as ideias das estruturas de repetições 
  (`loops`) na programação, como `for` ou `while`, por exemplo. 
- **rvest**; para raspagem de dados em documentos html. É o principal pacote de 
  nosso minicurso.
...
Note que forma carregados 8 (oito) pacotes:
...
Essa é uma abordagem válida e simplifica a escrita dos códigos, tornando-o menos
"verboso".
Uma desvantagem, porém, é que, quando se é iniciante, nem sempre conhecemos de 
qual pacote pertence determinada função, o que pode atrapalhar os estudos de cada
pacote.
...
bem delimitadas, trazendo clareza em cada linha de código.
...
Grosso modo, é uma tabela pra lá de especial.
...
Para nossos fins, basta-nos saber como formar uma `tibble` e como visualizá-la.
...
classificados como "character", mas, obviamente, gostaríamos que ele fosse 
"numeric".
...
Ao veificarmos a classe, percebemos que os dados passaram a ser numéricos:
...
O pacpte [dplyr](https://dplyr.tidyverse.org/) fornece uma estruturação de funções
...
Portanto, só usaremos algumas pucas funções desse pacote.

Uma delas é a função **mutate()**.

Em inglês, numa tradução livre, a palavra *mutate* referece a "mudança", "alteração",
etc.
...
Ele acrescentará uma nova variável (que daremos o nome `diferenca`) à `tabela_livros_mod`.
...
"tomada de decisão".
Se o "preço novo" for 30% mais barato do que o "preço antigo", quero que apareça 
a mensagem "compre".
Caso contrário, quero que apareça a mensagem "aguarde".
...
Antes de prosseguirmos, retorne ao código em html e procure encontrar *tag* como
...
Sim ... os dados não chegam para nós "aruumadinhos"!
Ao contrário, na maioria das vezes, os conjuntos de dados possuem caracteres numa
forma não formatados, ou simplesmente não estão como desejamos organizar.

De fato, quando fazemos uma "raspagem de dados", estamos trabalhando com 
"fragmentos de textos".
Manipular esses fragmentos são fundamentais para que as análises sejam realizadas
sem muitos "sustos" (geralmente não sai como desejamos quando, por exemplo, um
caractere especial esteja presente num local onde não desejávamos).

Seremos diretos e abordaremos as funções desse pacote que usaremos nesse curso.
...
Há uma função no `stringr` que extrai uma sequência de caracteres que possuem
determinado padrão.
...
Ótimo!
Mas, se desejamos apenas os valores numéricos, precisamos eliminar três caracteres: o "R", o "`$`" e o "espaço vazio"!).
...
- *start = ...*, é o ponto que iniciaremos a extração. Devemos colocar aqui o
   número da posição do caractere que desejamos começar!
- *end = ...*, é o ponto que desejamos parar. Se não colocarmos nada, 
   subentende-se que iremos extrair até o final da *string*.
...
O nome do nosso diretório (pasta) será "download_arquivos" e ele é feito assim:
...
lista de link fosse consideravelmente maior, ficaria inviável.
...
Em nosso exemplo, a função é `download.file` e os argumentos são `arg1 = links_R`
e `arg1 = download_arquivos/nomes_links_R`.
...
APENAS passas dois argumentos, mas agora aparecerá um terceito: `mode = "wb"`.
...
Agora vamos para a raspagem dos dados!!!