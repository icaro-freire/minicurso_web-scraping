...
Entretanto, usaremos, apenas, oito desses pacotes nesse minicurso:
...
- **lubridate**; é uma ferramenta importantíssima para cálculos e conversões
com data. Mas, apenas usaremos a função `dmy()` que converte um conjunto de
caracteres "xx/yy/zzzz" em datas, realmente.
...
Para conhecermos a função `dmy()` do pacote [`lubridate`](https://lubridate.tidyverse.org/),
vamos considerar o seguite vetor:

```{r}
possiveis_datas <- c("01/01/2020", "01/01/2021", "01/01/2022")
possiveis_datas
```

À primeira vista, parecem datas, não? 
O formato seria: "dia/mês/ano" (que em inglês é "day/month/year", daí o nome da 
função: `dmy()`); mas será que está no formato adequado?

```{r}
class(possiveis_datas)
```

Não são datas, mas "caracteres".
Para transformar em datas, fazemos:

```{r}
datas_verdadeiras <- lubridate::dmy(possiveis_datas)

datas_verdadeiras
```

Agora, verificando sua classe, encontrados como saída "datas", realmente.

```{r}
class(datas_verdadeiras)
```

Pode parecer um detalhe, mas isso para uso em cálculos que envolvem datas é muito 
importante.
Não é possível realizarmos cálculos com caractéres.
...
Base, da seguinte forma: `seq(1, 10)`), por simplicidade.
...
Ele criará os dois diretórios, caso não existam, ou criará o subdiretório `pasta_arquivos` se `downloads` já exista.
)
