...
Então, não será objetivo nosso um aprofundamento na linguagem `R` (que é algo 
muito maior e demandaria muito tempo).
Apenas falaremos, brevemente, sobre aspectos essencias dessa linguagem que 
utilizaremos nos pacotes do *tidyverse*.

Esse texto foi planejado para aprenderemos alguns aspectos básicos da linguagem 
`R` que nos fornecerão instrumentos para desenvolvermos nossa raspagem de dados
...
Mas, não para por aí!
O `R` também é uma excelente calculadora estatística!
...
- **destino/nome-e-extensao-do-arquivo**: pasta (diretório) e o nome do arquivo
  explicitando a sua extensão. A barra "/" delimita o caminho relativo;
- **mode**: por padrão é "w", para arquivos de texto plano. Mas, se for, por exemplo,
  arquivos `.pdf`, deve-se colocar o argumento "wb" (arquivos binários).
...
Vamos fazer o *download* do arquivo de texto que se encontra na página:

<p align='center'>
  https://nbcgib.uesc.br/lec/download/R/ir/C0_apresentacao.R
</p>

Vamos atribuir à variável `url_uesc` esse endereço de *link* (notem as aspas):

```{r}
url_uesc <- "https://nbcgib.uesc.br/lec/download/R/ir/C0_apresentacao.R"
```

Além disso, vamos determinar que o arquivo "C0_apresentacao.R" seja colocado na
pasta "arquivos_teste".
Logo, podemos fazer a seguinte atribuição para o caminho relativo desse arquivo:

```{r}
destino_arquivo <- "arquivos_teste/C0_apresentacao.R"
```
...
Agora, para fazermos o *download* corretamente, precisamos colocar o argrumento
`mode = "wb"`:
...
> Variáveis **quantitativas** tabmbém são chamadas de **numéricas**. </br>
Variáveis **qualitativas** também são chamdas de **categóricas**.
...
As listas são estruturações mais gerais do `R`, pois podem conter outras 
estruturas já vistas, bem como outra lista!!
